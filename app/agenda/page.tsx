...
// Placeholder data
const placeholderAgenda: AccordionItem[] = [
  {
    _id: '1',
    title: 'News#1 Title',
...
export default async function AgendaPage() {
  let agendaItems: AccordionItem[] = placeholderAgenda
  ...
  try {
    ...
    if (Array.isArray(sanityAgenda) && sanityAgenda.length > 0) {
      agendaItems = sanityAgenda as AccordionItem[]
    }
  } catch (error) {
  ...
  return (
    <AccordionList
      items={agendaItems}
      emptyMessage="No agenda items yet. Add some in Sanity Studio!"
    />
  )
}
